package com.yuex.util.util.jwt;
...
public class JwtUtil {
    ...
    public static String sign(String token, String secret) {
        return sign(token, secret, EXPIRE_TIME);
    }

    /**
     * @param expiresInMillis JWT 过期时长（毫秒），应与 Redis 中登录会话 TTL 一致，避免「JWT 仍有效但会话已失效」
     */
    public static String sign(String token, String secret, long expiresInMillis) {
        Date date = new Date(System.currentTimeMillis() + expiresInMillis);
        Algorithm algorithm = Algorithm.HMAC256(secret);
